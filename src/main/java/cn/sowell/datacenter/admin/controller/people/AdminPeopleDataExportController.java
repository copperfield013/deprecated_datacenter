...
public class AdminPeopleDataExportController {
	...
	public ResponseJSON doImport(@RequestBody JsonRequest jReq, HttpSession session){
	...
	public ResponseEntity<byte[]> download(@PathVariable String uuid){
		...
		try {
			...
			headers.setContentDispositionFormData("attachment", new String(
					("人口导出数据-" + dateFormat.format(new Date(), "yyyyMMddHHmmss") + ".xlsx").getBytes("UTF-8"),
					"iso-8859-1"));
