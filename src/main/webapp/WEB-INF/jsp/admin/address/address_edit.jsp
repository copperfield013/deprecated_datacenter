...
<div id="address-edit" class="zpage">
	<h1 class="zpage-title">修改地址分词</h1>					
	<form id="address-edit-form" action="admin/address/doEdit" >
					...
					<input type="hidden" id="laterPart" name="splitedAddressEntity.laterPart" value="${splitedAddressEntity.laterPart }"/>
						<div class="margin-t10">
							...
							<a class="primary inline-middle margin-l20 " href="admin/address/edit_name?addressStr=${splitedAddressEntity.name }">修改</a>					
						</div>						
						<div class=" margin-t10">
										...
										<div class="inline-middle margin-r10" >
											<select class="level" name="elements[${i.index }].level">
												<c:forEach items="${levelNameMap }" var="levelName">											
													<option value="${levelName.key }" ${addressElement.level==levelName.key?'selected=true':''}>${levelName.value }</option>											
												</c:forEach>
											</select>
										</div>
...
<script>
	seajs.use(['dialog', 'utils'], function(Dialog, Utils){
		...
		//增加一级  依次修改之后的name属性
		$('#address-edit-form',$page).on('click','.address-add-level',function(){
		...
		//去除当前行 依次修改之后的name属性
		$('#address-edit-form',$page).on('click','.address-remove-level',function(){
			var NEXTELELIST = $(this).closest('.address-participle-item').nextAll();
