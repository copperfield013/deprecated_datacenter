...
<div id="peopledata-model">
    <nav>
        <form class="form-inline" action="admin/peopledata/model">
            ...
            <a class="btn btn-primary tab" href="admin/peopledata/modelAdd/0" title="创建模板" target="people_outputAdd_0">创建</a>
        </form>
    </nav>
    <div class="row list-area">
        <table class="table">
            ...
            <c:forEach items="${list }" var="item" varStatus="i">
                <tr>
                    <td>${i.index + 1 }</td>
                    <td><a class="tab" href="admin/peopledata/modelDetail/${item.id }"
                           target="people_detail_${item.id }" title="详情-${item.modelName }">${item.modelName }</a></td>
                    <td>
                        <a href="admin/peopledata/modelAdd/${item.id }" class="tab"
