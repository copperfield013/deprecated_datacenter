...
<div id="peopledata-output">
	<nav>
		<form class="form-inline" action="admin/peopledata/output">
			...
			<a class="btn btn-primary tab" href="admin/peopledata/outputAdd/0" title="创建模板" target="peopledata_outputAdd" >创建</a>
				...
				<c:forEach items="${list }" var="item" varStatus="i">
					<tr>
						<td><input name="model" id="${i.index + 1 }" value="${item.id }" style="opacity: 1; position: static; height: 13px; border: medium none; background-color: rgb(255, 255, 255); cursor: auto;" type="radio">
							${i.index + 1 }</td>
						<td><a class="tab" href="admin/peopledata/outputDetail/${item.id }" target="peopledata_detail_${item.id }" title="详情-${item.modelName }">${item.modelName }</a></td>
						<td>
							<a href="admin/peopledata/outputAdd/${item.id }" class="tab" target="peopledata_outputAdd" title="修改-${item.modelName }">修改</a>
