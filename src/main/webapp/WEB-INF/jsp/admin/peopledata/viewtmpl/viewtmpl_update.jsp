...
<c:choose>
	<c:when test="${tmpl == null }">
		<c:set var="title" value="创建人口信息模板" />
	</c:when>
	<c:otherwise>
		<c:set var="title" value="修改人口信息模板-${tmpl.name }" />
	</c:otherwise>
</c:choose>
<title>${title }</title>
<link type="text/css" rel="stylesheet" href="media/admin/peopledata/css/viewtmpl-update.css" />
<div id="viewtempl-update-${tmpl.tmplId }" class="viewtmpl-update">
...
<script>
	seajs.use(['peopledata/js/viewtmpl-update.js', 'ajax'], function(ViewTmpl, Ajax){
		var $page = $('#viewtempl-update-${tmpl.tmplId }');
		console.log($page);
